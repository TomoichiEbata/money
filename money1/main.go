/*
	"hitachi"を、"toshiba","jfe"と一斉変換すれば、対応可能
*/
...
func main() {
	...
	hitachi_db, err := sql.Open("postgres", "user=postgres password=password host=localhost port=15432 dbname=hitachi sslmode=disable")
	if err != nil {
		log.Fatal("OpenError: ", err)
	}
	defer hitachi_db.Close()

	add_day := "2021/6/2"

	cash := 1000000 // 100万円からスタート
	//potential_asset := 0.0 //潜在的資金
	has_hitachi := 0 // 持っていない:0 持っている:1
	old_close := 0   //
	...
	for i := 0; i < 365; i++ {
		...
		rows, err := hitachi_db.Query(select_string)
		if err != nil {
			log.Fatal(err)
		}
		defer rows.Close()
